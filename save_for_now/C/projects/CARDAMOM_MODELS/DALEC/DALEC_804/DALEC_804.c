...
int DALEC_804(DATA DATA, double const *pars)
{
...
double fl=(log(pars[30])-log(pars[30]-1))/2;
...
/* x (1 + a* P/P0)/(1+a)*/
FLUXES[f+1]=exp(pars[9]*0.5*(DATA.MET[m+2]+DATA.MET[m+1]-DATA.meantemp))*((DATA.MET[m+8]/DATA.meanprec-1)*pars[31]+1);
/*respiration auto*/
...
/*Water pool = Water pool - runoff + prec (mm/day) - ET*/
	...
	for (nn=0;nn<4;nn++){FLUXES[f+23+nn] = POOLS[nxp+nn]*DATA.MET[m+6]*(1-CF[nn])*(1-pars[29])/deltat;}
