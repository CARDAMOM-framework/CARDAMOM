...
int DALEC_820(DATA DATA, double const *pars)
{
...
/*Mortality factor*/
mf=fmin(POOLS[p+6]/(pars[25]*pars[33]),1);
...
FLUXES[f+9] = POOLS[p+1]*(1-pow(1-FLUXES[f+8],deltat))/mf/deltat;
/*wood litter production*/       
FLUXES[f+10] = POOLS[p+3]*(1-pow(1-pars[6-1],deltat))/mf/deltat;
/*root litter production*/
FLUXES[f+11] = POOLS[p+2]*(1-pow(1-pars[7-1],deltat))/mf/deltat;
