...
int DALEC_1200(DATA DATA, double const *pars)
{
...
/*Water pool = Water pool - runoff + prec (mm/day) - ET*/
	...
	/*FOSSIL FUEL*/
	/*Trend = % per day*/
	FLUXES[f+30] =pars[33]*(1 + log(pars[34])*deltat*n);
	/*Net ecosystem exchange + Fires + FF*/
	NEE[n]=-FLUXES[f+0]+FLUXES[f+2]+FLUXES[f+12]+FLUXES[f+13]+FLUXES[f+16] + FLUXES[f+30];
