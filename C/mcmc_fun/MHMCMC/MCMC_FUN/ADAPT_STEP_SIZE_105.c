#pragma once
int ADAPT_STEP_SIZE_105(double *PARSALL, PARAMETER_INFO PI, COUNTERS N, MCMC_OPTIONS MCO){
...
int n;
double fac=(1-(double)N.ACC/(double)MCO.nOUT/2)*(MCO.nADAPT/100)/100;
double  adapfac=fac+1;
...
if (N.ACCRATE<0.23){adapfac=1.0/adapfac;}
...
if (N.ACCLOC>3){
	...
	for (p=0;p<PI.npars;p++){
		...
		PI.stepsize[p]=(PI.stepsize[p]/adapfac*(1-fac/10) + norparstd*fac/10)*adapfac;
