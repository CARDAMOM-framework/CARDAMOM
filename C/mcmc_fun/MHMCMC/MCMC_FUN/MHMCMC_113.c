...
#include "NORMPARS.c"
#include "ADAPT_STEP_SIZE_113.c"
#include "STEP.c"
#include "STEP_WITH_BOUNCE_113.c"
...
while (N.ACC < MCO.nOUT && (P<0 || MCO.nWRITE>0 || N.ACC==0)){
	/*take a step*/
	STEP_WITH_BOUNCE_113(PARS0,PARS,PI,N,MCO);
	...
	if (N.ITER % MCO.nADAPT==0){
		...
		ADAPT_STEP_SIZE_113(PARSALL,PI,&N,MCO);}
	...
	if (MCO.nPRINT>0 && N.ITER % MCO.nPRINT==0){
		...
		printf("Local (Global) Acceptance rate %3.1f%% (%3.1f%%)\n",N.ACCRATE*100, (double)N.ACC/(double)N.ITER*100);
		printf("Log Likelihood %e\n",P0);
		meanstepsize=0;
		for (n=0;n<PI.npars;n++){meanstepsize+=N.parstdev[n]/(double)(PI.npars);}
