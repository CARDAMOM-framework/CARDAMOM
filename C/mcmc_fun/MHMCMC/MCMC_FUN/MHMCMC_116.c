...
while (N.ACC < MCO.nOUT && (P<0 || MCO.nWRITE>0 || N.ACC==0)){
	/*take a step*/
	withinrange=STEP_115(PARS0,PARS,PI,N,MCO);
	...
	if (P-P0>log((double)random()/RAND_MAX)){
		...
		if (MCO.nWRITE>0 && (N.ACC % MCO.nWRITE)==0){

		WRITE_RESULTS(PARS,P,PI,MCO);
	...
	if (N.ACC % MCO.nADAPT==0 && asw==1){
		...
		if (MCO.fADAPT*(double)MCO.nOUT>(double)N.ACC){
			ADAPT_STEP_SIZE_116(PARSALL,PI,&N,MCO);}
		/*adapt switch: allow adaptation after parameters accepted again*/
		asw=0;
		}
	...
	if (MCO.nPRINT>0 && N.ITER % MCO.nPRINT==0){
		...
		printf("Total Accepted = %d out of %d (rate = %3.3f%%)\n",N.ACC,MCO.nOUT,100*(double)N.ACC/(double)N.ITER);
