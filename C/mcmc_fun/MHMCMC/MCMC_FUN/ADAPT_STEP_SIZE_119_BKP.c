...
/*statfun_incremental_cov_mat(double **CM, int Npars, int Nsamples, double *CMmean, double *newsample, double ar)*/
	...
	double *parmean=calloc(PI.npars, sizeof(double));
	int pp;
	for (p=0;p<PI.npars;p++){for (pp=0;pp<PI.npars;pp++){CM[p][pp]=N->parcovariance[p][pp];}}
	for (p=0;p<PI.npars;p++){parmean[p]=N->parmean[p];}
	...
	if (choldiag==0){
	printf("CM = [");
        tempfun_print_mat(CM,PI.npars);
        printf("];\n x =[");
        tempfun_print_vec(norparveci,PI.npars);
        printf("];\n M = [");
        tempfun_print_vec(parmean,PI.npars);
        printf("];\n N=%i;\n",N->covsamplecount-1);
        printf("CMnew=[");
	tempfun_print_mat(N->parcovariance,PI.npars);
	printf("];");
	exit(1);}
	...
	choldiag=cholesky(N->parcovariance,PI.npars,CCM);
        if (choldiag==0){
	printf("Par removed\n");
        printf("CM = [");
        tempfun_print_mat(CM,PI.npars);
