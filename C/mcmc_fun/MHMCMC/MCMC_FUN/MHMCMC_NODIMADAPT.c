...
while (N.ACC < MCO.nOUT && (P<0 || MCO.nWRITE>0 || N.ACC==0)){
	/*take a step*/
	STEP_WITH_BOUNCE_NN(PARS0,PARS,PI);
	...
	if (N.ITER % MCO.nADAPT==0){
		...
		adapfac=1+(N.ACCRATE-0.23)*(1-(double)N.ACC*2/(double)MCO.nOUT)/10;
		for (n=0;n<PI.npars;n++){PI.stepsize[n]=PI.stepsize[n]*adapfac;
if (PI.stepsize[n]<MCO.minstepsize){PI.stepsize[n]=MCO.minstepsize;}
}
}
