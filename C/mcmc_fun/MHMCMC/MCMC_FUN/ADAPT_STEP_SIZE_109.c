#pragma once
int ADAPT_STEP_SIZE_109(double *PARSALL, PARAMETER_INFO PI, COUNTERS N, MCMC_OPTIONS MCO){
...
int n;
double fac=1, adapfac=(1-(double)N.ACC/(double)MCO.nOUT/2)*(MCO.nADAPT/100)/10+1;
...
if(N.ACCRATE>0.23){fac=adapfac;
for (n=0;n<PI.npars;n++){PI.stepsize[n]=PI.stepsize[n]*fac;}}
/*if(N.ACCRATE<0.23){
for (n=0;n<PI.npars;n++){PI.stepsize[n]=PI.stepsize[n]/pow(adapfac,1);}}
*/
...
if (N.ACCLOC>3){
	...
	for (p=0;p<PI.npars;p++){
		...
		{PI.stepsize[p]=PI.stepsize[p]*adapfac;}
		 ...
		 if (PI.stepsize[p]/fac>norparstd*2.38/sqrt((double)PI.npars))
                {PI.stepsize[p]=PI.stepsize[p]/adapfac;}
