...
if (DATA->ncdf_data.CH4.Uncertainty_Threshold<0){DATA->ncdf_data.CH4.Uncertainty_Threshold=1e-5;}; /*TO ADDRESS: other fields use "0" threshold as default*/
	...
	/*Static Elements: 1-100 - use as many as nbeded*/
...
printf("About to read cbf binary file...\n");
	...
	/*binary file mcmc options (nbed to add all options HERE except inout files)*/
	DATA->edc_random_search=(int)statdat[10];
	DATA->gppiav=(int)statdat[11];
	DATA->laiiav=(int)statdat[12];
	DATA->nbe_annual_unc=statdat[13];
	...
	for (n=0;n<DATA->nodays+1;n++){
		...
		if (obsline[3]>-9998){DATA->nabgb=DATA->nabgb+1;}}
	     ...
	     /*   if (DATA->noobs>11){DATA->NBEunc[n]=obsline[11];
	        if (obsline[11]>-9998){DATA->nnbeunc=DATA->nnbeunc+1;}}*/  /*shuang by default there is not a field for NBEunc in CBF.OBS*/
