 ...
 */
  double ** ncdf_read_double_2D(int ncid, const char * varName, size_t * dimLen ){
	int varID;
	int numberOfDims;
	int * dimensionIDs = {-1,-1}; //array where we store the dimensionIDs. default to -1
	//Attempt to locate the id of the approprate variable
	if ((retval = nc_inq_varid(ncid, varName, &varID))){
		ERR(retval);
	}
	...
	size_t * startIndexes = {0,0}; //This is just the indexes we want to start at, which we pass to nc_get_vara. We will begin at 0,0, and increment only the first dim
	size_t * readCount = {1,-1}; //This is just the number of elements we want to read at a time, which we pass to nc_get_vara.
...
int CARDAMOM_READ_NETCDF_DATA(char *filename,NETCDF_DATA *DATA)
{
	...
	NETCDF_DATA->ABGB=ncdf_read_single_double_var(ncid, "ABGB", -9999.0);
	NETCDF_DATA->CH4=ncdf_read_single_double_var(ncid, "CH4", -9999.0);
	NETCDF_DATA->EDC=ncdf_read_single_double_var(ncid, "EDC", -9999.0);
	NETCDF_DATA->EDCDIAG=ncdf_read_single_double_var(ncid, "EDCDIAG", -9999.0);
	NETCDF_DATA->ET=ncdf_read_single_double_var(ncid, "ET", -9999.0);
	NETCDF_DATA->EWT=ncdf_read_single_double_var(ncid, "EWT", -9999.0);
	NETCDF_DATA->GPP.values=ncdf_read_double_var(ncid, "GPP", &(NETCDF_DATA->GPP.length));

	ncdf_read_double_attr

	NETCDF_DATA->ID=ncdf_read_single_double_var(ncid, "ID", -9999.0);
	NETCDF_DATA->LAI.values=ncdf_read_double_var(ncid, "LAI", &(NETCDF_DATA->LAI.length));
