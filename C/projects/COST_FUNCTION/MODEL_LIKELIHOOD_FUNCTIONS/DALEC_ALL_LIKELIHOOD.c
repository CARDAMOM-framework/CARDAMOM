...
if (O->SUPPORT_SIF_OBS){   ML[LI.SIF]=CARDAMOM_TIMESERIES_OBS_LIKELIHOOD(&D.ncdf_data.SIF, D.M_GPP);printf("in SIF TS OBS LIKELIHOOD\n");}; // shuang
...
if (O->SUPPORT_iniSOM_OBS){   ML[LI.PEQ_iniSOM]=CARDAMOM_SINGLE_OBS_LIKELIHOOD(&D.ncdf_data.PEQ_iniSOM, D.M_PEQ_iniSOM);printf("in iniSOM single OBS LIKELIHOOD\n");};
