#pragma once
int EDC1_900(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD)
{
...
double torfol=pars[4];//1/(pars[4]*365.25);
...
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[15-1]==1)) & (pars[26]<pars[27] | pars[26]<pars[28])){EDC=0;EDCD->PASSFAIL[15-1]=0;}
...
/*EDC No 16: Tmmin < Tmmax*/
//if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[16-1]==1)) & (pars[13]>pars[14])){EDC=0;EDCD->PASSFAIL[16-1]=0;}

/*EDC No 17: Photoperiodmmin < Photoperiodmmax*/
//if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[17-1]==1)) & (pars[15]>pars[23])){EDC=0;EDCD->PASSFAIL[17-1]=0;}


/*EDC No 18: VPDmmin < VPDmmax*/
//if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[18-1]==1)) & (pars[24]>pars[25])){EDC=0;EDCD->PASSFAIL[18-1]=0;}
