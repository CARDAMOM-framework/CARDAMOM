...
int DALEC_1001(DATA DATA, double const *pars)
{
...
/*Water pool = Water pool - runoff + prec (mm/day) - ET*/
	/*printf("%2.1f\n",POOLS[p+6]);*/
    /*Surface runoff*/
    FLUXES[f+32] = deltat*(DATA.MET[m+8] - pars[36]*(1 - exp(-DATA.MET[m+8]/pars[36])));
    /*PAW total runoff*/
  ...
  /*Plant-available water ODE*/
	POOLS[nxp+6]=POOLS[p+6] + (DATA.MET[m+8] - FLUXES[f+28] - FLUXES[f+29] - FLUXES[f+30] - FLUXES[f+32])*deltat;
