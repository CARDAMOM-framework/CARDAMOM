...
while (PEDC==0 && n<DATA->ncdf_data.TIME_INDEX.length  && k==0){
  ...
  p+=1;
    ...
    //     // Changed loop variable to 'i' so it doesn't overwrite 'p' (which you use below for Fail check)
    //     for (int i = 0; i < DATA->nopools; i++){   
    //         // FIXED: Prevent divide-by-zero if a pool hasn't been attempted yet
    //         double percent = (attempt_count[i] == 0) ? 0.0 : 100 * (double)pass_count[i] / (double)attempt_count[i];
            
    //         printf("pool %i; Attempt = %i, Pass = %i, Percent = %2.2f%%\n", i, attempt_count[i], pass_count[i], percent);
    //     }
    // }
