#pragma once
int EDC1_1004(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD)
{
...
EDCD->nedc=100;
int n; for (n=0;n<EDCD->nedc;n++){DATA.M_EDCD[n]=1;}
...
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[1-1]==1)) & (pars[8]>pars[7])){EDC=0;DATA.M_EDCD[1-1]=0;}

/*EDC CHECK NO 2*/
/*Litter2SOM greater than SOM to Atm. rate*/
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[2-1]==1)) & (pars[0]<pars[8])){EDC=0;DATA.M_EDCD[2-1]=0;}

/*EDC CHECK NO 3*/
/*TOR_FOL faster than TOR_WOOD */
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[3-1]==1)) & (pars[5]>torfol)){EDC=0;DATA.M_EDCD[3-1]=0;}

/*EDC CHECK NO 4*/
/*Root turnover greater than SOM turnover at meantemp*/
/*same as this*/
/*\text{EDC 4: }(1-\pavii)^{365} > \Pi_{i=1}^{365} (1-\paix \tratei)*/
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[4-1]==1)) & (pars[6]<pars[8]*exp(pars[9]*meantemp))){EDC=0;DATA.M_EDCD[4-1]=0;}
...
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[5-1]==1)) & ((ffol+flab)>5*froot | (ffol+flab)*5<froot)){EDC=0;DATA.M_EDCD=0;}

/*EDC No 16* Foliage CF> wood CF & SOM CF*/
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[16-1]==1)) & (pars[27]<pars[28] | pars[27]<pars[29])){EDC=0;DATA.M_EDCD=0;}
