...
#include "DALEC_1300_INDICES.c"
#include "PARS_INFO_1300.c"
...
int DALEC_1300_FLUX_SOURCES_SINKS(DALEC * DALECmodel){
    

    struct DALEC_1300_FLUXES F=DALEC_1300_FLUXES;
    struct DALEC_1300_POOLS S=DALEC_1300_POOLS;
...
int DALEC_1300(DATA DATA, double const *pars){

    



struct DALEC_1300_PARAMETERS P=DALEC_1300_PARAMETERS;
struct DALEC_1300_FLUXES F=DALEC_1300_FLUXES;
struct DALEC_1300_POOLS S=DALEC_1300_POOLS;
...
double *POOLS=DATA.M_POOLS;
  ...
  /*Nitrogen pools*/
  POOLS[S.N_soil]=POOLS[S.C_som]*pars[P.mineralization_factor];
...
//Nitrogen demand//
FLUXES[f+F.n_demand] = (1./pars[P.living_tissue_cn_ratio])*(FLUXES[f+F.gpp]-FLUXES[f+F.resp_auto_maint]-FLUXES[f+F.foliar_prod]-FLUXES[f+F.root_prod]-FLUXES[f+F.wood_prod]-FLUXES[f+F.resp_auto_growth]);

//Cost of AM uptake of Nitrogen//
FLUXES[f+F.cost_am] = (1. - pars[P.perecm])*((pars[P.am_cost_par_n]/POOLS[S.N_soil]) + (pars[P.am_cost_par_c]/POOLS[S.C_root]))

//Cost of ECM uptake of Nitrogen//
FLUXES[f+F.cost_ecm] = pars[P.perecm]*((pars[P.ecm_cost_par_n]/POOLS[S.N_soil]) + (pars[P.ecm_cost_par_c]/POOLS[S.C_root]))

// Total cost of Nitrogen acquistion via mycorrhizae - 1./cost_myc = 1./cost_am + 1./cost_ecm
FLUXES[f+F.cost_myc] = (FLUXES[f+F.cost_am]*FLUXES[f+F.cost_ecm])/(FLUXES[f+F.cost_am]+FLUXES[f+F.cost_ecm])

//*Actual C spent in N acquistion is NPP - NSC
FLUXES[f+F.actual_c_spent] = FLUXES[f+F.gpp]-FLUXES[f+F.resp_auto_maint] - FLUXES[f+F.lab_prod] 

FLUXES[f+F.N_uptake] = FLUXES[f+F.actual_c_spent]/FLUXES[f+F.cost_myc]
...
/*total pool transfers (no fires yet)*/
        ...
        //Nitrogen pool//
        POOLS[nxp+S.N_soil] = POOLS[nxp+S.N_soil] - (FLUXES[f+F.N_uptake])*deltat;
...
int DALEC_1300_MODCONFIG(DALEC * DALECmodel, DATA * DATA){


struct DALEC_1300_PARAMETERS P=DALEC_1300_PARAMETERS;
struct DALEC_1300_FLUXES F=DALEC_1300_FLUXES;
struct DALEC_1300_POOLS S=DALEC_1300_POOLS;
struct DALEC_1300_EDCs E=DALEC_1300_EDCs;




DALECmodel->dalec=DALEC_1300;
...
DALEC_1300_FLUX_SOURCES_SINKS(DALECmodel);




//Define PARS_INFO here (ranges, and eventually names, etc)
PARS_INFO_1300(DALECmodel);
