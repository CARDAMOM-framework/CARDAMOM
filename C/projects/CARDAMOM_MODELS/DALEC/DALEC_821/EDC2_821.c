...
int EDC2_821(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD)
{
...
& ((fabs(log(Rs))>log(EQFnew)) || (fabs(Rs-Rm)>etol)))
...
/*of each constraint*/
EDCD->EDCPROB[7-1+n]=-0.5*pow(log(Rs)/log(EQFnew),2);/*-0.5*pow((Rs-Rm)/etol,2);*/
...
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[7-1+n]==1)) & (pars[23]>MPOOLS[6])){EDC=ipow(0,EDCD->SWITCH[14-1]);EDCD->PASSFAIL[14-1]=0;}
...
int pidx[]={16,17,18,19,20,21,24};

for (n=0;n<nopools;n++){if ((EDC==1 || DIAG==1) & ((MPOOLS[n])>parmax[pidx[n]])){EDC=0;EDCD->PASSFAIL[35-1]=0;}}


int PEDC;
/* G. Quetin added a minimum pool size check to negative check*/
double minpool = .0001;
	...
	while ((n<nopools) & (EDC==1 || DIAG==1)){
		...
		while ((nn<nodays+1) & (PEDC==1)){
			if ((POOLS[n+nn*nopools]<minpool) || isnan(POOLS[n+nn*nopools])==1){
				EDC=0;PEDC=0;EDCD->PASSFAIL[35+n]=0;
			}
			nn=nn+1;
		};
	n=n+1;
