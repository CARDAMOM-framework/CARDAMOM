...
int DALEC_1103_MODCONFIG(DALEC * DALECmodel){
...
int DALEC_1103(DATA DATA, double const *pars){
...
double beta = fmin(POOLS[p+S.H2O_PAW]/pars[P.wilting],1.)*(1-pow(fmin(fmax(VPD[n],0),pars[P.vpd_ref])/pars[P.vpd_ref],pars[P.vpd_exp]));
       // beta = fmin(beta,g);
