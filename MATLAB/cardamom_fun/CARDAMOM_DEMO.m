...
hist(CBR.PARS(:,37));ylabel('Frequency');xlabel('Medlyn g1');
subplot(2,2,4);
%hist(CBR.PARS(:,17));ylabel('Frequency');xlabel('Leaf C Mass per area [gC/m2]');
hist(CBR.PARS(:,38));ylabel('Frequency');xlabel('Vcmax25');

%Step 5. Plot outputs
figure(1);clf
subplot(2,2,1);set(gca,'FontSize',6);
%plot(1:72,CBR.FLUXES(:,:,33)+CBR.FLUXES(:,:,34));ylabel('Transpiration + Evap [kgH2O/m2/day]');xlabel('Months since Jan 2010');
