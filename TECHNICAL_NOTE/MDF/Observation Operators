...
8) If this is a completely new variable for the CBF, then in CARDAMOM_READ_BINARY_DATA.c, add a memory allocation (eg., DATA->M_GPP=calloc(Ntimesteps,sizeof(double));) NOTE: can trigger bus error if skipped
